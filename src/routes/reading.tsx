...
function ReadingPage() {
  ...
  const scenes: { icon: LucideIcon; title: string; tone: string; text: string }[] = [
    ...
    { icon: Coins, title: "财运投资", tone: "text-[var(--gold)] bg-[color-mix(in_oklab,var(--gold)_15%,transparent)]",
      text: cesuanData?.caiyun?.sanshishu_caiyun?.detail_desc || "正财平稳，偏财起伏。请避开高杠杆，将一笔小额预算分批配置于稳健品类。" },
    { icon: HeartHandshake, title: "情感婚姻", tone: "text-[var(--cinnabar)] bg-[color-mix(in_oklab,var(--cinnabar)_12%,transparent)]",
    ...
    { icon: GraduationCap, title: "学业成长", tone: "text-[var(--jade)] bg-[color-mix(in_oklab,var(--jade)_12%,transparent)]",
